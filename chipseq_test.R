...
#1.Trimming adapter and Filter low quality reads
for(i in all_samples){
  infile1 <- paste0(raw_data_path,i,'_R1_001.fastq.gz')
  infile2 <- paste0(raw_data_path,i,'_R2_001.fastq.gz')
  out_file1 <- paste0(trim_data_path,i,'_R1_001.fastq.gz')
  out_file2 <- paste0(trim_data_path,i,'_R2_001.fastq.gz')
  adapter_seq <- unlist(strsplit(i,'_'))[3]
  out_files <- paste0(trim_data_path,i,
                      c('_R1_paired.fastq.gz','_R1_unpaired.fastq.gz','_R2_paired.fastq.gz','_R2_unpaired.fastq.gz')
                      ,collapse = ' ')
  system(paste('sbatch /storage/htc/birchlerlab/CENH3_ChIP-seq/scripts/bash_sub.sbatch cutadapt -a',
               adapter_seq,'-A',adapter_seq,'-o',out_file1,'-p',out_file2,infile1,infile2))
}

#2.Filter low quality reads
for(i in all_samples){
  infile1 <- paste0(trim_data_path,i,'_R1_001.fastq.gz')
  infile2 <- paste0(trim_data_path,i,'_R2_001.fastq.gz')
...
#4. Map reads to genome (B73 + B cenromere sequences, should bind together) and merge bam files for replicates
for (i in all_samples) {
